...
class AGCRN(nn.Module):
    def __init__(self, args, precomputed_embeddings=None):
        ...
        if precomputed_embeddings is not None:
            assert precomputed_embeddings.shape == (self.num_node, args.embed_dim), \
                "Precomputed embeddings must have shape (num_nodes, embed_dim)"
            self.node_embeddings = nn.Parameter(precomputed_embeddings, requires_grad=False)
        else:
